...
void SbpCollector::CollectUniverse(SbpSignature& sbp_) {
...
void SbpCollector::CollectUniverse(SbpNode<SbpSignature>* sbp_node) {
  for (auto& sbp_ : sbp_node->SbpSignatureObjList) { CollectUniverse(sbp_); }
}
// Collect all the possible Sbp Parallel from a SbpGraph
void SbpCollector::CollectUniverse(SbpGraph<SbpSignature>& sbp_graph) {
...
SbpNode<SbpSignature>* SbpCollector::InitializePorxy(
...
void SbpCollector::InitializeCopyCostFromNode2Proxy(SbpNode<SbpSignature>* sbp_proxy,
...
void SbpCollector::ProxySbpCandidate(
    ...
    HashMap<std::string, Algorithm::SbpNode<SbpSignature>*>& op_name2sbp_node,
    Algorithm::SbpGraph<SbpSignature>& sbp_graph, HashMap<std::string, bool>& op_name2is_fixed) {
  ...
  op_graph.ForEachNode([&](const OpNode* node) {
    ...
    if (op_name2is_fixed[node->op().op_name()]) return;
    ...
    for (const std::string& ibn : node->op().input_bns()) {
      ...
      // not building proxy for fixed opertors
      if (op_name2is_fixed[producer.op().op_name()]) return;
      ...
      Algorithm::SbpNode<SbpSignature>* consumer_sbp_node = op_name2sbp_node[node->op().op_name()];
  ...
  for (auto& lbi7groups : producer_lbi2consumer_bn2sbp_set) {
    ...
    // Todo: coding
    // std::cout << "insert " << lbi.DebugString() << " after " << producer_name << std::endl;
  }
}
